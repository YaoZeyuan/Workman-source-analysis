...
class Worker
{
    ...
    protected static function checkSapiEnv()
    {
        ...
        *   可能返回的值包括
        *       aolserver、apache、 apache2filter、apache2handler、 
        *       caudium、cgi （直到 PHP 5.3）, cgi-fcgi、cli、 
        *       cli-server、 continuity、embed、fpm-fcgi、
        *       isapi、litespeed、 milter、nsapi、 phttpd、pi3web、
    ...
    protected static function init()
    {
        // Start file.
        /**
         * debug_backtrace会返回以函数所在文件顺序为入栈顺序的一系列debug数组，
         * count($backtrace) - 1 即为真正执行的函数所在的文件对应的debug数组
         * 取file，获取所在文件路径
         * 类似于__FILE__常量
         */
    ...
    protected static function initWorkers()
    {
        foreach (self::$_workers as $worker) {
            ...
            if (empty($worker->user)) {
                ...
                /* @zeyuany Linux系统中，以uid为0的账号为root账号 */
    ...
    protected static function initId()
    {
        foreach (self::$_workers as $worker_id => $worker) {
            /* @zeyuany 用0填充$worker->count个键值对（以初始化work池） */
    ...
    {
        /* @zeyuany workman即使是在这样一个仅有两行的方法里也用了可以理解的变量名。#好习惯，早养成# */
    ...
    {
        /* @zeyuany 翻译：通过黑科技(控制台带颜色输出)，输出启动信息 */
    ...
    protected static function parseCommand()
    {
        ...
        /* @zeyuany posix_kill => 向$master_pic指定进程发送信号0,发送成功返回true,失败返回false; PS:没有信号0,所以,只要能发送,pid一定存在。 */
        ...
        /* @zeyuany 我勒个我去。。。居然还能这么写- - */
        // execute command.
        switch ($command) {
            case 'kill':
                exec("ps aux | grep $start_file | grep -v grep | awk '{print $2}' |xargs kill -SIGINT");
                /* @zeyuany 休息一百万微秒 => 休息一秒 */
                /* @zeyuany 论高性能服务器的构建(没开玩笑,usleep可能真的比sleep准确) */
                usleep(100000);
    ...
    protected static function installSignal()
    {
        ...
        // ignore
        /* @zeyuany SIG_IGN = signal_ignore，忽略该信号，转由系统处理 */
        /* @zeyuany 这是高并发服务器提高性能的一个技巧 via http://blog.csdn.net/liuchao1986105/article/details/6440896 */
    ...
    protected static function daemonize()
    {
        ...
        /* @zeyuany 重设文件掩模 */
        /* @zeyuany umask(0) => 对于创建的每一个文件chmod(777) */
        umask(0);
        $pid = pcntl_fork();
        if (-1 === $pid) {
        ...
        /* @zeyuany 翻译:再次fork以避免 System V4 系统(传说中的BSD v4)再次取回终端控制权 */
        $pid = pcntl_fork();
        if (-1 === $pid) {
            throw new Exception("fork fail");
        } elseif (0 !== $pid) {
    ...
    protected static function monitorWorkers()
    {
        ...
        while (1) {
            ...
            /* @zeyuany 在每个周期中都要调用，以处理该循环中收到的信号（本质上是把信号交给注册的信号处理函数） */
    ...
    public function listen()
    {
        ...
        if ($this->transport === 'unix') {
            ...
            if (!is_file($address)) {
                /* @zeyuany 注册一个方法，当函数结束时(执行完毕或exit)，自动调用该方法 */
        ...
        /* @zeyuany Nagle算法，为tcp/ip的默认启用的算法，只有客户端包积累数量达到一定程度后再发送，以尽量避免包头比包内容还长的情况，对于高交互的C/S型程序用处不大 */
    ...
    {
        /* @zeyuany lcfirst — 使一个字符串的第一个字符小写 */
