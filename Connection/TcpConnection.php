...
class TcpConnection extends ConnectionInterface
{
    ...
    public function __construct($socket, $remote_address = '')
    {
        ...
        self::$statistics['connection_count']++;/* @zeyuany $statistics是继承自interface中的静态变量，所以可以直接使用 */
        $this->id      = $this->_id = self::$_idRecorder++;
        $this->_socket = $socket;
        stream_set_blocking($this->_socket, 0);/*  @zeyuany 设置$socket为非阻塞模式，使用后可直接调取 */
                ...
                if (!is_resource($this->_socket) || feof($this->_socket)) {
                    ...
                    if ($this->onError) {
                        try {
                            ...
                            /* @zeyuany Error是PHP7中新引入的异常类型,将部分Error转为异常进行处理 */
                            echo $e;
                            exit(250);
                        }
                    }
